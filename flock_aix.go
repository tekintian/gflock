...
func (f *GFlock) lock(locked *bool, flag lockType) error {
	f.m.Lock()
	defer f.m.Unlock()
...
func (f *GFlock) doLock(cmd cmdType, lt lockType, blocking bool) (bool, error) {
...
func (f *GFlock) doUnlock() (err error) {
	var owner *GFlock
	mu.Lock()
	ino, ok := inodes[f]
	if ok {
...
func (f *GFlock) try(locked *bool, flag lockType) (bool, error) {
	f.m.Lock()
	defer f.m.Unlock()
