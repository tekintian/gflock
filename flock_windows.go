...
func (f *GFlock) lock(locked *bool, flag uint32) error {
	f.m.Lock()
	defer f.m.Unlock()
...
// get the lock, we also set the *GFlock instance as being shared-locked.
func (f *GFlock) TryRLock() (bool, error) {
	return f.try(&f.r, winLockfileSharedLock)
}

func (f *GFlock) try(locked *bool, flag uint32) (bool, error) {
	f.m.Lock()
	defer f.m.Unlock()
