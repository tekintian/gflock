...
func ExampleGFLocked() {
	f := gflock.New(os.TempDir() + "/go-lock.lock")
...
func ExampleGFlockTryLock() {
	// should probably put these in /var/lock
	fileLock := gflock.New(os.TempDir() + "/go-lock.lock")
...
func ExampleGFlockTryLockContext() {
	// should probably put these in /var/lock
	fileLock := gflock.New(os.TempDir() + "/go-lock.lock")
